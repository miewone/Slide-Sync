...
export function readGeometry(node,inherited=[]){const xs=[node,...inherited].map(xfrm).filter(Boolean);if(!xs.length)return null;const g={x:pick(xs,'off','x',NaN),y:pick(xs,'off','y',NaN),w:pick(xs,'ext','cx',NaN),h:pick(xs,'ext','cy',NaN),rot:pick(xs,null,'rot',0)/60000,flipH:pick(xs,null,'flipH',0)===1,flipV:pick(xs,null,'flipV',0)===1};Object.assign(g,TableGeometry.fromXml(node));if(![g.x,g.y,g.w,g.h,g.rot].every(Number.isFinite))return null;g.chX=pick(xs,'chOff','x',0);g.chY=pick(xs,'chOff','y',0);g.chW=pick(xs,'chExt','cx',g.w);g.chH=pick(xs,'chExt','cy',g.h);return g;}
