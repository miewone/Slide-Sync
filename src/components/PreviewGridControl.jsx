 ...
 * @param {object} props Current grid, disabled state, onChange({columns,rows}) callback and optional idPrefix for a second workspace.
 */
export function PreviewGridControl({grid,disabled,onChange,idPrefix=''}) {
  useLanguage();
  ...
  const input=(key,value,setValue)=> <input id={`${idPrefix}preview-grid-${key}`} type="number" min="1" max="8" step="1"
  ...
  return <div className="preview-grid-control" role="group" aria-label={t('previewGrid.label')}>
    ...
    <button id={`${idPrefix}preview-grid-default`} type="button" disabled={disabled} aria-pressed={grid.columns===null} onClick={()=>onChange({columns:null,rows:null})}>{t('previewGrid.default')}</button>
    <span className="preview-grid-divider" aria-hidden="true">·</span>
    <button id={`${idPrefix}preview-grid-2`} type="button" disabled={disabled} aria-pressed={grid.columns===2&&grid.rows===2} onClick={()=>choose(2)}>2<span className="preview-grid-times">×</span>2</button>
    <span className="preview-grid-divider" aria-hidden="true">·</span>
    <button id={`${idPrefix}preview-grid-4`} type="button" disabled={disabled} aria-pressed={grid.columns===4&&grid.rows===4} onClick={()=>choose(4)}>4<span className="preview-grid-times">×</span>4</button>
    <span className="preview-grid-divider" aria-hidden="true">·</span>
