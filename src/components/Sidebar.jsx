...
const SlideRow = memo(function SlideRow({index, title, checked, disabled, onCheck, matched, hovered}) {
  ...
  return <label ref={row} data-slide-index={index} data-preview-hover={hovered?'true':undefined} className={`slide-item${checked ? ' checked' : ''}${matched ? ' search-match' : ''}${hovered ? ' preview-hover' : ''}`} title={title}>
    ...
    <span className="slide-number">{String(index + 1).padStart(2, '0')}</span><span className="slide-label">{title}</span>{matched && <span className="slide-match-label">{t('Sidebar.1')}</span>}
  </label>;
});
...
export function Sidebar() {
  ...
  return <aside className="sidebar">
    ...
    <SlideSearch/>
    <div id="slide-list" className="slide-list">{slides.length ? slides.map(slide => <SlideRow key={slide.index} {...slide} disabled={busy} hovered={hoveredSlide===slide.index} matched={matches.has(slide.index)} onCheck={commands.setSlideChecked}/>) : <p className="muted">{t('Sidebar.8')}</p>}</div>
