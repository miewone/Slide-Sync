...
export async function checkTextFormat(browser,origin) {
  ...
  assert.equal(await browser.evaluate(`document.querySelector('#fit-width-unwrap').checked`),false);
  assert.equal(await browser.evaluate(`document.querySelector('#fit-text-width').disabled`),false);
  const originalWidth=await browser.evaluate(`document.querySelector('#slide-0 iframe').contentDocument.querySelector('[data-pptx-mover="${result.id}"]').firstElementChild.style.width`);
  await browser.evaluate(`document.querySelector('#fit-width-unwrap').click();document.querySelector('#fit-text-width').click()`);
  await browser.until(`!document.querySelector('#fit-text-width').disabled&&document.querySelector('#status').textContent.includes('가로를 내용에')`,'width fitted');
  assert.equal(await browser.evaluate(`document.querySelector('#slide-0 iframe').contentDocument.querySelector('[data-pptx-mover="${result.id}"] .text-wrapper').style.whiteSpace`),'pre');
  const fittedWidth=await browser.evaluate(`document.querySelector('#slide-0 iframe').contentDocument.querySelector('[data-pptx-mover="${result.id}"]').firstElementChild.style.width`);
  assert.notEqual(fittedWidth,originalWidth);
  assert.equal(await browser.evaluate(`document.querySelector('#slide-0 iframe').contentDocument.querySelector('[data-pptx-mover="${result.id}"] .text-wrapper').style.width`),fittedWidth);
  await browser.evaluate(`document.querySelector('#undo').click()`);
  assert.equal(await browser.evaluate(`document.querySelector('#slide-0 iframe').contentDocument.querySelector('[data-pptx-mover="${result.id}"]').firstElementChild.style.width`),originalWidth);
  await browser.evaluate(`document.querySelector('#redo').click()`);
  assert.equal(await browser.evaluate(`document.querySelector('#slide-0 iframe').contentDocument.querySelector('[data-pptx-mover="${result.id}"]').firstElementChild.style.width`),fittedWidth);
  await browser.evaluate(`document.querySelector('#undo').click()`);
  await browser.evaluate(`document.querySelector('#fit-width-background').click();document.querySelector('#fit-text-width').click()`);
  await browser.until(`!document.querySelector('#fit-text-width').disabled&&document.querySelector('#status').textContent.includes('가로를 내용에')`,'background width fitted');
  await browser.evaluate(`document.querySelector('#fit-width-background').click()`);
